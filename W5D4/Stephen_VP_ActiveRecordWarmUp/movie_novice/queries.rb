...
def star_wars
  ...
  # hint: use 'select' and 'where'
  Movie.select(:id,:title,:yr).where("movies.title LIKE '%Star War%'")
end
...
def below_average_years
  ...
  Movie.where("movies.score < 5").select("COUNT(movies.title) AS bad_movies", :yr).group(:yr).order("bad_movies DESC")
end
...
def uma_movies
  ...
  Movie.select(:id,:title,:yr).joins(:actors).where("actors.name = 'Uma Thurman'").order("movies.yr ASC")
end
